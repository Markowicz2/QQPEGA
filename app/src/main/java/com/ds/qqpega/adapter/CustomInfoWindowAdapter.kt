...
class CustomInfoWindowAdapter(context: Context) : GoogleMap.InfoWindowAdapter {
    ...
    private fun rendowWindowText(marker: Marker, view: View) {
        ...
        val start_hd = LocalTime.parse(parts?.get(3) )
        val end_hd = LocalTime.parse(parts?.get(4))
        val start_hs = LocalTime.parse(parts?.get(5))
        val end_hs = LocalTime.parse(parts?.get(6))
        val start_ht = LocalTime.parse(parts?.get(7))
        val end_ht = LocalTime.parse(parts?.get(8))
        val start_hq = LocalTime.parse(parts?.get(9))
        val end_hq = LocalTime.parse(parts?.get(10))
        val start_hqi = LocalTime.parse(parts?.get(11))
        val end_hqi = LocalTime.parse(parts?.get(12))
        val start_hse = LocalTime.parse(parts?.get(13))
        val end_hse = LocalTime.parse(parts?.get(14))
        val start_hsa = LocalTime.parse(parts?.get(15))
        val end_hsa = LocalTime.parse(parts?.get(16))
        ...
        tvDistancia.text = "Distancia: "+ (parts?.get(0) ) +" Km"
        tvEntrada.text = parts?.get(1)
        if(parts?.get(1) ==""){
            tvEntrada.text = "Sem entrada"
        }else
        {
            tvEntrada.text = "Entrada/Couvert: "+parts?.get(1)
        }

        var link = parts?.get(2)
